...
// Determine the best path for user-config.json
// Priority: 1. /data (Fly.io volume), 2. utils/ (local/Docker), 3. root (legacy)
...
function getOverridePath() {
  ...
  return (fs.existsSync(NEW_OVERRIDE_PATH) || !fs.existsSync(LEGACY_OVERRIDE_PATH)) 
    ? NEW_OVERRIDE_PATH 
    : LEGACY_OVERRIDE_PATH;
}

const OVERRIDE_PATH = getOverridePath();
...
function normalizeConfig(base) {
  ...
  if (cfg.enableProxy === undefined) cfg.enableProxy = false;
...
function loadConfig() {
  ...
  providerNames.forEach(name => {
    const flag = `enable${name.charAt(0).toUpperCase() + name.slice(1)}Provider`;
    const envName = `ENABLE_${name.toUpperCase()}_PROVIDER`;
    envCfg[flag] = process.env[envName];
  });
